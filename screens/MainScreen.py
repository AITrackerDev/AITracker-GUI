...
class MainScreen(ctk.CTkFrame):
    ...
    def __init__(self, root, screen_changer):
        ...
        _about_button = ctk.CTkButton(self, text='Help', corner_radius=10, command=lambda: self.screen_changer('AboutScreen'))
        _settings_button = ctk.CTkButton(self, text='Settings', corner_radius=10, command=lambda: self.screen_changer('SettingsScreen'))
        ...
        if not DEBUG:
            ...
            for device in _usb_devices:
                if device.idVendor == 0x0403 and device.idProduct == 0x6014:
                    _device_found = True
                    self.screen_changer('LaunchScreen')
